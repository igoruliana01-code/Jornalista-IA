...
$("#dashboard-empty").classList.add("hidden");$("#results").classList.remove("hidden");const c=Math.max(0,Math.min(100,Number(d.confidence)||0));const st=d.primary_status||d.status||"—";
 ...
 <article class="card evidence-card">${mh("⌁","Rastreamento da Evidência")}<p class="muted">Cada afirmação abaixo precisa apontar para a fonte que realmente a sustenta. “Confirmado” não é sinônimo de “a IA acha que está certo”.</p><div class="evidence-list">${evidenceList(d.evidence_records)}</div></article>
