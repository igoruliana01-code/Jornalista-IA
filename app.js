...
$("#dashboard-empty").classList.add("hidden");$("#results").classList.remove("hidden");const c=Math.max(0,Math.min(100,Number(d.confidence)||0));const st=d.primary_status||d.status||"—";
 ...
 <article class="card mini">${mh("✓","Evidências Diretas")}<ul>${list(d.direct_evidence)}</ul></article>
 ...
 <div class="wide-grid"><article class="card mini purple-card">${mh("↗","Principais Fontes")}${sources}</article><article class="card mini">${mh("◇","Qualidade das Fontes")}<p>${esc(d.source_quality||"—")}</p></article><article class="card mini yellow-card">${mh("◇","Contexto — não confundir com conflito")}<ul>${list(d.context_evidence)}</ul></article><article class="card mini green-card">${mh("⚠","Conflitos Reais")}${d.contradiction_evidence?.length?`<ul>${list(d.contradiction_evidence)}</ul>`:`<p>🟢 Nenhuma contradição direta identificada.</p>`}</article></div>
